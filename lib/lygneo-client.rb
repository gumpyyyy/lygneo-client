module LygneoClient
  ...
  require 'lygneo-client/railtie' if defined?(Rails)

  autoload :App,            File.join('lygneo-client', 'app')
  autoload :AccessToken,    File.join('lygneo-client', 'access_token')
  autoload :ResourceServer, File.join('lygneo-client', 'resource_server')
  ...
  # @see [LygneoClient#getter]
  # @see [LygneoClient#setter]
  # @return [void]
  ...
  def self.config(&block)
    ...
    if defined?(Rails) && Rails.env == "production" && !@test_mode
      unless self.verify_manifest
        $stderr.puts <<-HELP
******** Your Lygneo Client is not setup in test mode and ***********
******** Your manifest.json does not match your configuration **********

Please do the following:
1. Double check your configuration (ex. config/lygneo_client.rb).
...
HELP
  ...
  #
  # @return [Addressable::URI] The url of the server using LygneoClient.
  def self.application_base_url
    if @application_base_url.match(/^localhost:\d+/)
      @application_base_url = LygneoClient.scheme + "://" + @application_base_url
    end
  ...
  def self.initialize_instance_variables
    ...
    @account_class = nil
    @account_creation_method = :find_or_create_with_lygneo
  end
  ...
  def self.permission(type, access, description)
    @permissions[type] = {:type => "LygneoClient::#{type.to_s.upcase}".constantize,
                          :access => "LygneoClient::#{access.to_s.upcase}".constantize,
  ...
  def self.write_manifest
    ...
    man_f.write(LygneoClient.package_manifest)
    man_f.close
  end
