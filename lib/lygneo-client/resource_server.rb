...
module LygneoClient
  ...
  class ResourceServer < ActiveRecord::Base
    ...
    # @param [String] host The location of a Lygneo server.
    # @raise [RegistrationError] If the Lygneo server returns a non-20x response code.
    ...
    def self.register(host)
      ...
      unless(response.present? && response.success?)
        message = "Failed to connect to Lygneo server: "
    ...
    def signable_string
      [ LygneoClient.application_base_url,
        full_host,
        Time.now.to_i,
    ...
    # @return [Addressable::URI] The URI of the Lygneo pod represented by this record.
    def full_host
    ...
    # @return [String] The registration endpoint of this Lygneo pod.
    def token_endpoint
      url = self.full_host
    ...
    # @return [String] Root API route of this Lygneo pod.
    def api_route
      url = self.full_host
      url.path = '/api/v0'
      url.to_s
